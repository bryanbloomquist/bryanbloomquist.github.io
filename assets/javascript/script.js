...
const welcomePage = () => {
  $(".greetings").empty();
  typed(".greetings", "Hi, I'm Bryan.  I make cool stuff happen on the internet.", 0);
}
...
let techSkillsPage = () => {
  toolbox.map(tool => {
    $("#toolbox").append(
      '<button type="button" class="toolbox-item" data-toggle="popover" title="'+tool.title+'" data-content="'+tool.content+'">' +
        ' <img class="skillimage" src="' + tool.src + '" alt="' + tool.alt + '" />' +
      '</div>'
    )
  })
}
